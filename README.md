...
- fix: 修复快速移动组件导致整个画布节点全部消失的问题（网页/桌面端）
  - 根因：拖动时高频同步节点位置到全局 store 触发重渲染，与 React Flow 内部拖拽状态竞争
  - 方案：画布只一次性读取 store 初始值，写回留在 useEffect 且不订阅值，消除重渲染竞争
- feat: 新增 LTE/NR 网络接口(NIC)数据源，支持基于 siteId/经纬度的跨制式地图对比
- chore: 分发聚焦 GitHub + 阿里云服务器，移除 Gitee 镜像引用
